...
async function getChapters() {
  const res = await fetch("http://127.0.0.1:8000/content/chapter/?class_name=nine&subject=english", { cache: "no-store" });
  if (!res.ok) return [];
  return res.json();
}
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "rgb(18,18,22)", color: "white" }}>
      <div className="max-w-3xl mx-auto p-4">
        <h1 className="text-3xl font-bold mb-4">English Chapters (Class Nine)</h1>
        <div className="space-y-3">
          {chapters.map((c: any) => (
            <Link key={c.id} href={`/vocational/nine/english/chapter/${c.id}`} className="block">
